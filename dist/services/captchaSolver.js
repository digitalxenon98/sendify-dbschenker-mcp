 ...
 */
function parsePuzzle(puzzleHeader) {
 ...
 */
function calculateTarget(puzzle) {
    if (puzzle.length < 15) {
 ...
 */
async function computeHash(nonce, puzzle) {
    if (puzzle.length < 32) {
 ...
 */
export async function solveCaptcha(puzzleHeader) {
    try {
        // Parse the puzzle
        ...
        // Solve each puzzle
        const solutions = await Promise.all(puzzles.map(async ({ jwt, puzzle }) => ({
            jwt,
...
export function validateSolutionFormat(solution) {
    try {
        ...
        return parsed.every(item => typeof item === "object" &&
 ...
 */
export function extractJWTsFromPuzzle(puzzleHeader) {
    const decoded = Buffer.from(puzzleHeader, "base64").toString("utf-8");
