...
If the required `Captcha-Solution` header is missing, the upstream service responds
deterministically with HTTP 429 and a `Captcha-Puzzle` response header. **CAPTCHA enforcement
occurs before any business-level validation, including reference existence checks.** This response
indicates an enforced system boundary rather than transient rate limiting.
...
The system performs a lightweight, client-side heuristic validation before making upstream requests.

This validation rejects references that are non-numeric or shorter than 8 digits, is
intentionally conservative, and only filters out obviously malformed input in order to
avoid unnecessary CAPTCHA-triggering requests.

This validation does not reflect DB Schenker's internal validation logic.

If the reference fails this check, the system returns a structured error response:
